...
namespace eCommerce_Csharp_Cards.Controllers {
    ...
    public class AmazonController : Controller {
        ...
        public async Task<IActionResult> Get () {
            try {
                IEnumerable<Cards> Cards = await _Cards.GetAmazon ();
        ...
        public async Task<IActionResult> Get (string Id) {
            try {
                ...
                Cards Card = await _Cards.GetAmazon (Id);
        ...
        public async Task<IActionResult> Post ([FromBody] Cards Card) {
            try {
                ...
                await _Cards.PostAmazon (Card);
                return Ok (JsonConvert.SerializeObject (await _Cards.GetAmazon()));
            } catch (Exception) {
        ...
        public async Task<IActionResult> Put (string Id, [FromBody] Cards Card) {
            try {
                ...
                if (!ModelState.IsValid) return StatusCode (StatusCodes.Status406NotAcceptable, ModelState);
                Card.Id = Id;
                var h = await _Cards.PutAmazon (Id, Card);
                if (h.MatchedCount > 0) return Ok (JsonConvert.SerializeObject (await _Cards.GetAmazon()));
        ...
        public async Task<IActionResult> Delete (string Id) {
            try {
                if (string.IsNullOrEmpty (Id) || Id.Length < 24) return StatusCode (StatusCodes.Status406NotAcceptable, "Id Invalid");
                var h = await _Cards.DeleteAmazon (Id);
                if (h.DeletedCount > 0) return Ok (JsonConvert.SerializeObject (await _Cards.GetAmazon()));
